<?php
class Plugg_Forum_Controller_Main_Index extends Plugg_Form_Controller
{
    ...
    protected function _doGetFormSettings(Sabai_Request $request, Sabai_Application_Response $response, array &$formStorage)
    {
        ...
        if (!$this->_submitable = $this->getUser()->isAuthenticated()) $this->_submitButtonName = null;
        ...
        $form = array(
            'topics' => array(
                '#type' => 'tableselect',
                '#header' => $this->getUser()->isAuthenticated() ? array('star' => '') : array(),
                '#multiple' => true,
                '#js_select' => true,
                '#options' => array(),
        ...
        // Fetch topics
        $sorts = array('sticky', $sort); // sticky topics always come first
        $orders = array('DESC', $order);
        if (isset($this->user)) {
            $pages = $this->getPluginModel()->Topic->paginateByUser($this->user->id, 20, $sorts, $orders);
            $response->setPageInfo(sprintf($this->_('Topics by %s'), $this->user->display_name));
        } else {
            $pages = $this->getPluginModel()->Topic->paginate(20, $sorts, $orders);
        }
        ...
        $topic_views = array();
        if ($this->getUser()->isAuthenticated() && $topics->count()) {

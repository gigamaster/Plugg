...
<?php   if ($messages_select != $select_k):?>
  <span><?php echo $this->LinkToRemote($select_v, 'plugg-content', array('path' => 'messages', 'params' => array('messages_select' => $select_k, 'messages_sortby' => $messages_sortby)));?></span>
  <span> | </span>
<?php   else:?>
...
<?php   endif;?>
<?php endforeach;?>
  <?php $this->_e('Sort by: ');echo $this->SelectToRemote('messages_sortby', $messages_sortby, 'plugg-content', $messages_sortby_allowed, array('path' => 'messages', 'params' => array('messages_select' => $messages_select)), $this->_('Go'));?>
  <span> | </span>
  <span><?php echo $this->LinkToRemote($this->_('Refresh'), 'plugg-content', array('path' => 'messages', 'params' => array('messages_select' => $messages_select, 'messages_sortby' => $messages_sortby, 'time' => time())));?></span>
</div>
<form action="<?php echo $this->Url(array('path' => 'messages/submit'));?>" method="post">
  <table class="plugg-horizontal">
    <thead>
      <tr>
        <th>
          <input id="plugg-messages-checkall" class="checkall plugg-messages-checkall2" type="checkbox" />
          <span><?php $this->_e('Sender');?></span>
        </th>
        <th colspan="2" width="50%"><?php $this->_e('Message');?></th>
        <th><?php $this->_e('Received at');?></th>
      </tr>
    </thead>
    <tfoot>
      <tr>
        ...
        <td colspan="2"><?php echo $this->PageNavRemote('plugg-content', $messages_pages, $messages_page->getPageNumber(), array('path' => 'messages', 'params' => array('messages_select' => $messages_select, 'messages_sortby' => $messages_sortby)));?></td>
      </tr>
    </tfoot>
    <tbody>
...
<?php   foreach ($messages->with('FromToUser') as $message): $message_sender = $message->FromToUser;?>
      <tr class="messages-message<?php if ($message->isRead()):?> shadow<?php endif;?>">
        ...
        <td colspan="2" class="messages-messages-content"><span class="messages-messages-title"><?php echo $this->LinkToRemote(mb_strimlength($message->title, 0, 100), 'plugg-content', array('path' => 'messages/' . $message->id));?></span></td>

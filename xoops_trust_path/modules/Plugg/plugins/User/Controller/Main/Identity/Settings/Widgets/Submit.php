<?php
class Plugg_User_Controller_Main_Identity_Settings_Widgets_Submit extends Sabai_Application_Controller
{
    function _doExecute(Sabai_Request $request, Sabai_Application_Response $response)
    {
        ...
        if (!Sabai_Token::validate($token_value, 'user_main_identity_settings_widgets_submit', false)) {
            $response->setError($this->_('Invalid request'), $url);
            return;
        }
        ...
        $acrive_widgets = $this->getPluginModel()->Activewidget->fetchByUser($this->identity->id, 0, 0, 'order', 'ASC');
        foreach ($acrive_widgets as $current_widget) {
    ...
    private function _createActiveWidget($request, $widgetId, $widgetData)
    {
        ...
        $widget->assignUser($this->identity);

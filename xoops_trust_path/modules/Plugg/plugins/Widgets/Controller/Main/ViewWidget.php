<?php
class Plugg_Widgets_Controller_Main_ViewWidget extends Sabai_Application_Controller
{
    protected function _doExecute(Sabai_Request $request, Sabai_Application_Response $response)
    {
        ...
        $widget_settings = $this->widget_plugin->widgetsGetWidgetSettings($this->widget->name);
        ...
        if (!$this->widget->isCacheable() // make sure widget content may be cached
            ...
            if (false === $widget_content = $this->widget_plugin->widgetsGetWidgetContent(
                $this->widget->name,
                $settings,
                $this->getUser()
            )) {
        ...
        $vars = array(
            'widget' => array(
                ...
                'path' => '/widgets/' . $this->widget_plugin->name . '/' . $this->widget->name,
            )
        );

        $response->setContent($this->RenderTemplate('widgets_main_viewwidget', $vars))->setPageInfo($active_widget->title);
    }

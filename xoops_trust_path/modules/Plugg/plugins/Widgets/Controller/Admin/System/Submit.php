<?php
class Plugg_Widgets_Controller_Admin_System_Submit extends Sabai_Application_Controller
{
    function _doExecute(Sabai_Request $request, Sabai_Application_Response $response)
    {
        ...
        if (!Sabai_Token::validate($token_value, 'widgets_admin_submit', false)) {
            $response->setError($this->_('Invalid request'), $url);
            return;
        }
        ...
        if ($widgets = $request->asArray('widgets')) {
            ...
            foreach ($widgets as $widget_order => $widget_id) {
                ...
                if (!is_numeric($widget_order)) {
                    $position = constant('Plugg_Widgets_Plugin::WIDGET_POSITION_' . $widget_order);
                    continue;
                }

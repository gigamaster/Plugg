<?php
class Plugg_Groups_Controller_Main_Group_Settings_Widgets_Submit extends Sabai_Application_Controller
{
    function _doExecute(Sabai_Request $request, Sabai_Application_Response $response)
    {
        $url = array('path' => 'settings/widgets');
        ...
        if (!Sabai_Token::validate($token_value, 'groups_main_group_manage_widgets_submit', false)) {
            $response->setError($this->_('Invalid request'), $url);
            return;
        }
        ...
        $active_widgets = $this->getPluginModel()->Activewidget->fetchByGroup($this->group->id, 0, 0, 'order', 'ASC');
        foreach ($active_widgets as $current_widget) {
        ...
        if ($widgets = $request->asArray('widgets')) {
            ...
            $position = Plugg_Groups_Plugin::WIDGET_POSITION_LEFT;
            foreach ($widgets as $widget_order => $widget_id) {
                ...
                if (!is_numeric($widget_order)) {
                    $position = constant('Plugg_Groups_Plugin::WIDGET_POSITION_' . $widget_order);
                    continue;
                }

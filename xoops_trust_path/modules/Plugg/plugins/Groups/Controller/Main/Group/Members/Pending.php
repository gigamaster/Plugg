<?php
class Plugg_Groups_Controller_Main_Group_Members_Pending extends Plugg_Form_Controller
{
    protected function _doGetFormSettings(Sabai_Request $request, Sabai_Application_Response $response, array &$formStorage)
    {
        ...
        $path = '/groups/' . $this->group->name . '/members/pending';
        ...
        $this->_successUrl = $this->group->getUrl('members/pending');
        ...
        $form[$this->_submitButtonName]['reject'] = array(
            ...
            '#submit' => array(array(array($this, 'reject'), array($request))),
            '#weight' => 1,
        );
        ...
        if ($pages->count() > 1) {
            $form['members']['#footer'] = $this->PageNavRemote(
                'plugg-groups-main-group-members-pending', $pages, $page->getPageNumber(), $this->getUrl($path, array('sort' => $sort, 'order' => $order))
            );
        }
    ...
    public function reject(Plugg_Form_Form $form, Sabai_Request $request)
    {
        if (empty($form->values['members'])) return true;

<?php
class Plugg_Aggregator_Controller_Main_Feeds_Feed_Edit extends Plugg_Form_Controller
{
    ...
    {   
        ...
        foreach (array('favicon_url', 'favicon_hide') as $key) {
            if (!isset($form->settings['favicon'][$key])) continue;
            
            $this->feed->$key = $form->values[$key];
        }
        
        if (isset($form->values['favicon_url']) && isset($form->settings['favicon']['favicon_url'])) {
            $this->feed->favicon_url = $form->values['favicon_url'];
        }
        
        if (isset($form->settings['favicon']['favicon_hide'])) {
